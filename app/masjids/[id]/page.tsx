...
import { MapPin, Navigation, Phone, Clock, CheckCircle2, Users, AlertTriangle, ExternalLink } from 'lucide-react'
...
export default function MasjidDetailPage() {
  ...
  const handleCheckin = () => {
    ...
    const stored = getItem<Record<string, CheckinData>>(KEYS.CHECKINS, {})
    const current = stored[masjid.id]?.lastReset === today ? stored[masjid.id] : { count: 0, lastReset: today }
    const updated = { ...stored, [masjid.id]: { count: current.count + 1, lastReset: today } }
