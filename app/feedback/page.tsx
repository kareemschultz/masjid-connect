...
const CATEGORIES = ['Bug Report', 'Feature Request', 'General Feedback', 'Community Suggestion']
...
export default function FeedbackPage() {
  ...
  const handleSubmit = () => {
    ...
    const entry = { name, email, category, message, rating, date: new Date().toISOString() }
    ...
    return (
      <div className="min-h-screen bg-[#0a0b14] pb-24">
        ...
        <div className="flex flex-col items-center justify-center px-6 py-20 text-center">
          ...
          <button onClick={() => { setSubmitted(false); setMessage('') }} className="mt-6 rounded-xl bg-emerald-600 px-6 py-2.5 text-sm font-semibold text-white active:scale-95 transition-transform">
            Send More Feedback
          </button>
        </div>
        <BottomNav />
  ...
  return (
    <div className="min-h-screen bg-[#0a0b14] pb-24">
      ...
      <div className="space-y-4 px-4 -mt-2">
        ...
        <button onClick={handleSubmit} disabled={!message.trim()} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 py-3.5 text-sm font-bold text-white transition-all active:scale-[0.98] disabled:opacity-40 disabled:active:scale-100">
